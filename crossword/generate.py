...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        # ensure every value x in domain of variable v has same length as v length
        for variable in self.domains:
            for value in set(self.domains[variable]):
                if len(value) != variable.length:
                    # remove value x from domain of variable v
    ...
    def revise(self, x, y):
        ...
        # check if variables have overlap
        if self.crossword.overlaps[x, y]:
            # get overlap
            i, j = self.crossword.overlaps[x, y]
            # if overlap, check arc consistency
            for x_word in set(self.domains[x]):
                remove = True
                for y_word in set(self.domains[y]):
                    if x_word[i] == y_word[j]:
                        remove = False
                
                if remove == True:
                    # remove value from x domain if not equal to y value
    ...
    def ac3(self, arcs=None):
        ...
        """
        # queue all arcs in csp (arcs)
        if arcs == None:
            # if arcs = None, set equal to all arcs in problem
            arcs = []
    ...
    def consistent(self, assignment):
        ...
        # every value is correct length
            ...
            # no conflicts between neighbours
    ...
    def order_domain_values(self, var, assignment):
        ...
        # for all values in domain of var
        for value in self.domains[var]:
            unordered_values[value] = 0
            # for neighboring variables not in assignment
            for neighbor in self.crossword.neighbors(var) - set(assignment):
                if value in self.domains[neighbor]:
                    unordered_values[value] += 1

        return sorted(unordered_values, key=unordered_values.get)
